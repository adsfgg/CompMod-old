...
Script.Load("lua/menu2/PlayerScreen/GUIMenuXPDisplay.lua")
...
local kCustomizeTitleFont = ReadOnly{family = "AgencyBold", size = 64}
...
local kTitleTopPadding = 115
local kSceneBottomPadding = 15
local kCustomizeTitleTopPad = 15
...
local function UpdateRightSideWidth(self)
    ...
    local rightWidth = math.max(totalWidth - leftWidth, 256)
...
function OpenPlayerProfile(self)
    ...
    steamId = Client.ConvertSteamId32To64(steamId)
...
function GUIMenuPlayerScreen:Initialize(params, errorDepth)
    ...
    self.skillTierIcon:SetSteamID64(Client.ConvertSteamId32To64(Client.GetSteamId()))
    ...
    self.xpDisplay = CreateGUIObject("xpDisplay", GUIMenuXPDisplay, self.leftSideContents)
    self.xpDisplay:HookEvent(self.leftSideContents, "OnSizeChanged", self.xpDisplay.SetWidth)
    self.xpDisplay:SetWidth(self.leftSideContents:GetSize().x)
    ...
    self.customizeTitle = CreateGUIObject("customizeTitle", GUIText, self.rightSide, 
    {
        ...
        align = "top",
        position = Vector(0, kCustomizeTitleTopPad, 0)
    })
    self.customizeTitle:AlignTopLeft()
    local tOffset = 120
    self.customizeScreen = CreateGUIObject("customizeScreen", GUIMenuCustomizeScreen, self.rightSide,
    {
        position = Vector(0, tOffset, 0), --TODO need to have offsets that jives with title size
        --size = Vector(1920, 1080 - tOffset, 0),
    })
    self.customizeScreen:AlignTop()
    --FIXME All of this MUST correctly maintain proper aspect ratio for client res...otherwise, image will be distorted, badly
    self.customizeScreen:HookEvent(self.customizeScreen, "OnSizeChanged", self.customizeScreen.SetWidth)
    self.customizeScreen:SetHeight(self.rightSide:GetSize().y - tOffset)
    self.customizeScreen:SetWidth(self.rightSide:GetSize().x)
